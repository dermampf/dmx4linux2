...
static void dmx512_receive_frame(struct dmx512suart_port *dmxport,
                  ...
                  if (e.data_inline.count < (512-dmxport->dmxsize))
                    {
                      ...
                      if (dmxport->dmx_locked && (dmxport->dmxsize == dmxport->dmx_lock_size))
                        {
                          dmx512_receive_frame(dmxport, dmxport->dmx_data, dmxport->dmxsize);
                          dmxport->dmxsize = 0;
                          dmxport->dmxstate = 0;
                        }
                    }
...
void dmx512suart_port_init(struct dmx512suart_port * dmxport, struct suart_pc16x50 * suart)
{
  dmxport->suart = suart;
  dmxport->dmxstate = 0;
  dmxport->dmxsize = 0;
...
int main ()
{
  ...
  struct suart_pc16x50 * suart = suart_pc16x50_create (&rxwaitqueue);
  if (!suart)
    {
  ...
  dmx512suart_port_init (&dmxport, suart);
  pthread_create(&rxfifo_handler_thread, NULL, dmx512suart_rxfifo_handler, &dmxport);
...

  {
    ...
    suart_put_txevents (&suart->suart, events, count);
  }

  {
    struct suart_event_s e;
    suart_event_query_baudrate(&e);
    suart_put_txevents (&suart->suart, &e, 1);
  }
  ...
  for (k=0; k<8; ++k)
    {
      ...
      suart_event_change_line(&event, SUART_LINE_DTR, SUART_LINE_DTR);
      suart_put_txevents (&suart->suart, &event, 1);
      usleep(500*1000);
      suart_event_change_line(&event, SUART_LINE_DTR, 0);
      suart_put_txevents (&suart->suart, &event, 1);
      usleep(500*1000);
    }
#endif
  ...
  sleep(1);
  run = 0;
  wake_up_interruptible(suart_rxwaitqueue (&suart->suart));
  pthread_join(rxfifo_handler_thread, 0);
  suart_pc16x50_delete(suart);  // does a suart_pc16x50_shutdown(suart);
