...
static int check_pattern(const unsigned char * data, const int size)
{
    int i;
    for (i = 0; i < 513; ++i)
...
int main (int argc , char **argv)
{
	...
	else if ((ret > 0) && (fds[0].revents & POLLIN))
	{
	    ...
	    {
		printf ("[%d]", count++);
		printf (" port:%d", frame.port);
		printf (" breaksize:%d", frame.breaksize);
		printf (" startcode:%d", frame.startcode);
		printf (" #slots:%d\n", frame.payload_size);
		int i;
		for (i = 0; i < frame.payload_size && i < 16; ++i)
		  printf (" %02X", frame.data[1+i]);
		if (frame.payload_size > 20)
		  printf (" ...");
		printf ("\n");
		printf ("  check: %s\n", check_pattern(frame.data, frame.payload_size+1) ? "OK" : "Failed");
	    }
	    else if (n == 0)
		printf ("no-frame\n");
	    else
